...
export async function savePageSeo(path: string, input: unknown): Promise<ActionResult<SeoMeta>> {
  try {
    ...
    // GUARDRAIL: Never allow duplicate titles like "Screw Wood - Screw Wood"
    if (data.title) {
      const cleanTitle = data.title.toLowerCase();
      if (cleanTitle.includes('screw wood - screw wood') || cleanTitle.includes('screw wood | screw wood')) {
        return {
          ok: false,
          error: 'Title guardrail violation: Duplicate brand name detected ("Screw Wood - Screw Wood"). Please revise.',
        };
      }
    }
