 ...
 */
export async function scanInternalLinks(baseUrl: string = 'https://www.screwwood.in'): Promise<LinkScanResult> {
  ...
  for (const route of routesToScan) {
    ...
    try {
      ...
      const res = await fetch(fullUrl, {
        method: 'GET',
        headers: { 'User-Agent': 'ScrewWoodLinkChecker/1.0' },
        cache: 'no-store',
      });
      ...
      while ((match = hrefRegex.exec(html)) !== null) {
        ...
        if (href.startsWith('/')) {
          ...
          isInternal = true;
        } else if (href.includes('screwwood.in') || href.includes('screwwood.com')) {
          targetUrl = href;
          isInternal = true;
        } else {
