...
const DEFAULT_SETTINGS: SeoSettings = {
  ...
  default_meta_desc: 'Turnkey house construction & luxury interiors in HBR Layout, Bengaluru. Transparent pricing, 415+ quality checks & 10-15 years structural warranty.',
...
export async function resolveSeo(rawPath: string): Promise<Metadata> {
  ...
  if (!titleStr) {
    if (path === '/') {
      titleStr = `${s.site_name} | Turnkey House Construction & Interiors in Bengaluru`;
    } else {
  ...
  const normalizedSiteUrl = (s.site_url || 'https://www.screwwood.in').replace(/\/$/, '');
