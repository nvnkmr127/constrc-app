...
export function buildGlobalSchemas(settings: SeoSettings): {
  ...
  const siteUrl = (settings.site_url || 'https://www.screwwood.in').replace(/\/$/, '');
  ...
  const organization = {
    ...
    name: settings.business_name || settings.site_name || 'Screw Wood',
    url: siteUrl,
    logo: logoUrl,
    email: settings.email || 'reachus@screwwood.in',
